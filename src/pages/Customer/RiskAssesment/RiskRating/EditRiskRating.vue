<template>
  <div>
    ...
    >
      <q-toolbar> Edit Risk Rating </q-toolbar>
      <q-btn
        flat
        round
        dense
      ...
      <div class="col-12 col-md-6 col-lg-6">
        <div class="column">
          ...
          <q-select
            ...
            dense
            clearable
            v-model="formData.likelyhood_before_control_value"
          ...
          <q-select
            ...
            dense
            clearable
            v-model="formData.severity_before_control_value"
            :options="ratingOptions"
          ...
          <q-select
            ...
            dense
            clearable
            v-model="formData.frequency_before_control_value"
            :options="ratingOptions"
      ...
      <div class="col-12 col-md-6 col-lg-6">
        <div class="column">
          ...
          <q-select
            ...
            dense
            clearable
            v-model="formData.likelyhood_after_control_value"
            :options="ratingOptions"
            label="Select Likelyhood"
            :rules="[
          ...
          <q-select
            ...
            dense
            clearable
            v-model="formData.severity_after_control_value"
            :options="ratingOptions"
            label="Select Severity"
            :rules="[
              (val) => !!val || 'Please enter Severity',
          ...
          <q-select
            ...
            dense
            clearable
            v-model="formData.frequency_after_control_value"
            :options="ratingOptions"
            label="Select Frequency"
            :rules="[
              (val) => !!val || 'Please enter Frequency',
...
export default {
  ...
  setup() {
    ...
    const { formData, filter, getItems } = storeToRefs(useStore);
    ...
    const onActivitySelect = () => {
      console.log("formData.value.activity_id", formData.value.activity_id);

      if (formData.value.activity_id !== null) {
        getHazards({ all: true, activity_id: formData.value.activity_id }).then(
          (response) => {
    ...
    const onHazardSelect = () => {
      if (formData.value.hazard_id !== null) {
        getRisks({
          all: true,
          filter: { hazard_id: formData.value.hazard_id },
        }).then((response) => {
    ...
    watch(
      () => {
        return formData.value.hazard_id;
      },
      (first, second) => {
        onHazardSelect();
        formData.value.risk_id = null;
      }
    );

    watch(
      () => {
        return formData.value.activity_id;
      },
      (first, second) => {
        formData.value.hazard_id = null;
      }
    );
